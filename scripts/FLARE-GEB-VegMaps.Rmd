...
## Vegetation
```{r}
tp<-raster("../data/VegetationRasters/FLARE-Eco-Forest-Percent_clip.tif")
```

```{r eval=FALSE, include=FALSE}
ft<-raster("../data/VegetationRasters/FLARE-Eco-Forest-Type.tif")
```
...
tp_crop <- crop(tp, extent(eco_ea))
```

```{r eval=FALSE, include=FALSE}

ft_crop <- mask(x=ft, mask=eco_ea)
```

```{r eval=FALSE, include=FALSE}

plot(eco_ea$geometry)
plot(tp_crop, add=TRUE)
plot(fires, add = TRUE)
```
...
ggplot()+
  layer_spatial(tp) +
  scale_fill_gradientn(colours=GreenPalette,  na.value = "transparent")
...
ggplot() +
     ...
     layer_spatial(tp_crop) +
  scale_fill_gradientn(colours=GreenPalette,limits=c(0,100), breaks=c(20, 40,  60,  80,  100),  na.value = "transparent") +
